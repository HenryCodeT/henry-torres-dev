...
export default function LlamitaChat() {
  const [isOpen, setIsOpen] = useState(false);
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      content: "¡Hola! I'm Llamita, Henry's AI assistant. I can answer questions about his skills, projects, services, and how to work with him. What would you like to know?",
      timestamp: new Date(),
    },
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage: Message = {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        content: "Lo siento! I'm having trouble connecting right now. Please try again or contact Henry directly at henry.torres@example.com",
        timestamp: new Date(),
      };
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            className="fixed bottom-24 right-6 z-50 w-96 h-[600px] bg-white rounded-3xl shadow-2xl flex flex-col overflow-hidden border-2 border-terracotta/20"
          ...
          >
            ...
            <div className="bg-gradient-to-r from-terracotta via-weaving-pink to-sky-blue p-4 text-white">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-lg">Llamita AI</h3>
                  <p className="text-xs text-white/90">Henry's Portfolio Assistant</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-stone-light/30">
                  ...
                  <div
                    className={`max-w-[80%] rounded-2xl px-4 py-2 ${
                      message.role === 'user'
                        ...
                        : 'bg-white text-foreground shadow-md border border-stone/10'
                    }`}
                  >
              ...
              {isLoading && (
                ...
                >
                  <div className="bg-white rounded-2xl px-4 py-2 shadow-md border border-stone/10">
            ...
            {messages.length === 1 && !isLoading && (
              <div className="px-4 py-2 bg-white border-t border-stone/10">
            ...
            {/* Input Form */}
            <form onSubmit={handleSubmit} className="p-4 bg-white border-t border-stone/10">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2 border border-stone/20 rounded-full focus:outline-none focus:ring-2 focus:ring-terracotta/50 text-sm"
                  disabled={isLoading}
                />
                <button
                  type="submit"
